...
namespace MedoNew.Admin
{
    public partial class AddProduct : System.Web.UI.Page
    {
        ...
        private void bind_Gridview()
        {
            ...
            ds = d1.GetDataSet("select * from ProductDetails");
            dgv.DataSource = ds;
            dgv.DataBind();
        }
        ...
        private bool isSubmit()
        {
            ...
            txtcreate.Text = Convert.ToString(Session["username"]);
            int i = 0;
            //txtcreate.Text = "0";
            int submit = d1.ExecuteQuery("insert into ProductDetails (Categoryid,Name,Prod_name,Image,Descp,user_id,Price,chemistid) values('" + DropDownList1.Text + "','" + txtname.Text + "','" + txtproname.Text + "','" + file_name + "','" + txtdesp.Text + "','" + txtcreate.Text + "','" + PriceN.Text + "','"+i+"')");
            if (submit > 0)
            {
                return true;    
            }
        ...
        protected void Save_Click(object sender, EventArgs e)
        {
            try
            {
                if (isSubmit())
                {
                    ClientScript.RegisterStartupScript(Page.GetType(), "validation", "<script language='javascript'>alert('Product Added Successfully...')</script>");
                }
        ...
        protected void dgv_RowDeleting(object sender, EventArgs e)
        {
            try
            {
                ...
                if (isDelete())
                {
                    ClientScript.RegisterStartupScript(Page.GetType(), "validation", "<script language='javascript'>alert('Product Deleted Successfully...')</script>");
                }
                else
                {
                    ClientScript.RegisterStartupScript(Page.GetType(), "validation", "<script language='javascript'>alert('Something is wrong. Please check...')</script>");
